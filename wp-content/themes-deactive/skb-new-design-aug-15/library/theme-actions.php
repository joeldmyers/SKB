...
add_action( 'wp_ajax_mark_offering_read', 'mark_offering_read' ); // ???? what is this?
add_action( 'wp_ajax_mark_investment_read', 'mark_investment_read');
...
add_action( 'wp_head', 'hide_from_search_bots' );


remove_action( 'wpua_before_avatar', 'wpua_do_before_avatar');
remove_action( 'wpua_after_avatar', 'wpua_do_after_avatar');

// redirect user from default wp pages 
add_action('login_form_login','skb_redirect_from_defaults');

function hide_from_search_bots() {
    
    if ( ! is_user_logged_in() ) {
    
        $hide_from_search = get_post_meta(get_the_ID(), 'hide_from_search', true );

        if ( $hide_from_search == 'Yes' ) {
...
function skb_redirect_from_defaults() {
...
function skb_scripts() {
  ...
  //  }
    ...
    wp_register_script( 'validate-js', get_template_directory_uri() . '/library/js/jquery.validate.js' );
    wp_enqueue_script( 'validate-js' );
    //wp_register_script( 'nice-file-js', get_template_directory_uri() . '/library/js/jquery.niceFileinput.js' );        
   // wp_enqueue_script( 'nice-file-js' );
    wp_register_script( 'skb-js', get_template_directory_uri() . '/library/js/skb.js');
    wp_enqueue_script( 'skb-js' );
    
    wp_register_script( 'bootstrap-table', get_template_directory_uri() . '/library/js/sortable.js');
    wp_enqueue_script( 'bootstrap-table' );

    //if ( is_page_template( 'content-aiq-form.php' ) ) {
...
function skb_styles() {   
    ...
    wp_register_style( 'bootstrap-table', get_template_directory_uri() . '/library/css/sortable-theme-dark.css');
...
//var_dump($user_id);die();
		$investments = get_user_meta( $user_id, 'read_list', FALSE );

		if ( ! in_array( $investment_id, $investments ) ) {

			update_user_meta( $user_id, 'read_list', $investment_id );
			update_post_meta( $investment_id, '_read_investment', $user_id, FALSE );
