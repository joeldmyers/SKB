...
// add function here for Pending Investments

function get_pending_investments() {
    ...
    foreach ( $investments as $investment ) {
        $offering_id = get_post_meta ($investment->ID, 'investment_offering_id', true);
        
        $date1 = get_the_time('U', $investment->ID);
        $date2 = $date1 + 14*24*60*60;
        
        $time = human_time_diff($date2,current_time('timestamp', 'U'));
        
        $html .= '<a href="' . $investment->guid . '" class="list-group-item clearfix">' . get_the_post_thumbnail( $offering_id, array( 90, 90), array( 'class' => 'pull-left' ) )  . '<h4 class="list-group-item-heading" style="margin-left: 100px;">' . $investment->post_title . '</h4></p></a>';
        $html .= '<p><small>Time remaining for approval: '.$time.'</small></p>';
        }
...
function send_to_docusign( $to_name, $to_email, $form_id, $sub_id ) {
	...
	switch ( $form_id ) {
		...
		case 14: // Corporation
			...
			break;
                   ...
                   $recipientEmail = 'test@checker.com';//$ninja_forms_processing->get_field_value( 1766 );
                   $docusign_redirect_url= home_url() . '/portal';//$ninja_forms_processing->get_field_value( 1766 );
                           $recipientName= 'Ryan Gatto';//$ninja_forms_processing->get_field_value( 1766 );
                    // $clientUserId      = '1';//$ninja_forms_processing->get_field_value( 1766 );
	...
	if ( $embedded ) {
            ...
            if (!isset($docusign_redirect_url)) {
...
/*
 * returns html for single investment right sidebar graphic
 */
function skb_get_offering_remaining_equity($post) { ?>
    
    <div class='col-lg-12 well'>
        <p class='small'>Equity available for this offering: 60% Funded</p>
        <div class="progress col-md-9">
            
            <?php 
            
            $remaining_equity = get_post_meta( $post->ID, 'investment_remaining_equity', true ); 
            $available_equity = get_post_meta( $post->ID, 'investment_available_equity', true );
           // $partner_equity = get_post_meta( $post->ID, 'investment_partner_equity', true );
           // $total_invested = get_post_meta( $post->ID, 'investment_total_invested', true );
            
            $remaining_equity_percent = ($remaining_equity / $available_equity) * 100;
            
            ?>
                <div class="progress-bar" role="progressbar" aria-valuenow="<?php echo (int)$remaining_equity_percent; ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo (int)$remaining_equity_percent; ?>%;">
                  <?php echo (int)$remaining_equity_percent; ?>%
                </div>
        ...
        <div class="col-md-3">
            <div class=""><?php echo $remaining_equity / 1000000; ?>M</div>
        </div>
    </div>
    <?php
 ...
 * returns html for single investment right sidebar graphic
 */
function skb_get_offering_total_equity($post) { ?>
    
    <div class='col-lg-12 well'>
        <p class='small'>Total equity invested: 90% Funded</p>
        <div class="progress col-md-9">
            ...
            $available_equity = get_post_meta( $post->ID, 'investment_available_equity', true );
            //$partner_equity = get_post_meta( $post->ID, 'investment_partner_equity', true );
            $total_equity = get_post_meta( $post->ID, 'investment_total_equity', true );
            
<<<<<<< HEAD
            $remaining_equity_percent = (isset($remaining_equity)) ? ($remaining_equity / $available_equity) * 100 : 0;
=======
            $remaining_equity_percent = ($remaining_equity / $available_equity) * 100;
>>>>>>> master
            
            ?>
                <div class="progress-bar" role="progressbar" aria-valuenow="<?php echo (int)$total_equity / 10000; ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo (int)$total_equity / 10000; ?>%;">
                  <?php echo (int)$total_equity / 10000; ?>%
                </div>
        </div>    
            
        <div class="col-md-3">
            <div class="col-md-2"><?php echo $available_equity / 1000000; ?>M</div>
        </div>
...
/*
 * returns days until end of cooldown or false if expired
 */
function getting_to_know_you_cooldown() {
    
    $user = wp_get_current_user();
    $cooldown = get_post_meta($user->ID, 'getting_to_know_you', true );
<<<<<<< HEAD
    $cooldown_start = time();
=======
    $cooldown_start = $cooldown;
>>>>>>> master
    $cooldown_end = $cooldown+(14*24*60*60);
    $cooldown_diff = human_time_diff($cooldown_start,$cooldown_end);
    
    if ( $cooldown != '' ) {
        echo "<p class='text-left alert alert-warning' style='padding-right:12px' title='Standard cooldown period is 2 weeks'>Getting to Know You Cooldown - {$cooldown_diff} remaining.</p>";
        return;
    }
